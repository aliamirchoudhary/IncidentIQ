...
| **Failure injection** | Deliberately breaking a dependency (bad API key, unreachable status-check URL, a D1 write failure, AI Gateway itself unreachable) and confirming the system degrades correctly rather than corrupting state or crashing | At minimum: LLM-provider-both-down (Stage 6/7), **AI Gateway itself unreachable — distinct from provider-down, since this triggers the direct-bypass fallback path and must log `route_used: "direct"` every time, per §7's LLM Strategy** (Stage 6/7), tool-call failure (Stage 10), one-agent-fails-mid-chain (Stage 13) |
